...
- Name: src/app/e2e/oauth/callback/test.ts >> /e2e/oauth/callback 回调页 >> 捕获回调查询参数
- Location: tests/e2e/src/app/e2e/oauth/callback/test.ts:16:3

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/e2e/oauth/callback?code=abc123&state=state-xyz&error=some-error
Call log:
  - navigating to "http://localhost:3000/e2e/oauth/callback?code=abc123&state=state-xyz&error=some-error", waiting until "domcontentloaded"
